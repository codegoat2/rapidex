...
import { refundAdminWithdrawal } from '../ledger/adminProfitService';
...
export function startWithdrawalWorker(): void {
  ...
  cron.schedule('*/15 * * * * *', () => {
    ...
    void processNextAdminWithdrawal().catch((err) => log.error({ err }, 'Admin withdrawal worker run failed'));
  });
}

async function processNextAdminWithdrawal(): Promise<void> {
  const withdrawal = await db.begin(async (sql) => {
    const rows = await sql<{
      ...
      amount: string;
      destination: string;
      source_exchanger_id: string;
      attempt_count: number;
    }[]>`
      SELECT id, asset, amount, destination, source_exchanger_id, attempt_count
    ...
    const [claimed] = await sql<typeof rows>`
      ...
      WHERE id = ${rows[0].id}
      RETURNING id, asset, amount, destination, source_exchanger_id, attempt_count
    `;
    return claimed;
  });
  ...
  try {
    const txId = await sendExchangerWithdrawal({
      ...
      exchangerId: withdrawal.source_exchanger_id,
    ...
    await db`
      UPDATE admin_withdrawals
      SET status = 'BROADCAST', tx_id = ${txId}, updated_at = NOW()
      WHERE id = ${withdrawal.id}
    `;
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    if (withdrawal.attempt_count >= 8) {
      await refundAdminWithdrawal(withdrawal.id, `Refund for failed admin withdrawal ${withdrawal.id}`);
    }
    const delay = Math.min(3600, 15 * 2 ** Math.min(withdrawal.attempt_count, 8));
    await db`
      UPDATE admin_withdrawals
      SET status = CASE WHEN attempt_count >= 8 THEN 'FAILED' ELSE 'PENDING' END,
          last_error = ${message}, next_attempt_at = NOW() + (${delay} * INTERVAL '1 second'), updated_at = NOW()
      WHERE id = ${withdrawal.id}
    `;
    log.error({ err: error, withdrawalId: withdrawal.id }, 'Admin withdrawal attempt failed');
  }
}
